...
---
  ...
  python render.py --marca steven-vallejo --locale all --layout web_hero
...
def localized_marca(marca: dict, locale: str = DEFAULT_LOCALE) -> dict:
    """Aplica overrides de idioma sin modificar el JSON cargado."""
    locale = (locale or DEFAULT_LOCALE).lower()
    if locale == DEFAULT_LOCALE:
        return marca

    locales = marca.get("locales", {})
    override = locales.get(locale, {})
    if not override:
        return marca

    merged = copy.deepcopy(marca)
...
def build_vars(marca: dict, layout: dict, locale: str = DEFAULT_LOCALE) -> dict:
    """Construye el diccionario de variables que se inyectan en el HTML."""
    marca = localized_marca(marca, locale)
    lid = layout["id"]
...
async def render_marca(
    ...
    marca_base = load_marca(marca_slug)
    ...
    async with async_playwright() as pw:
        ...
        try:
            ...
            for locale in locales:
                marca = localized_marca(marca_base, locale)
                locale_path = [] if (locale == DEFAULT_LOCALE and not multi_locale) else [locale]
                locale_label = "" if (locale == DEFAULT_LOCALE and not multi_locale) else f"{locale}/"

                for layout in layouts:
                    vars_dict   = build_vars(marca_base, layout, locale=locale)
                    h_new       = compute_hash(marca, layout, render_scale)
                    layout_id   = layout["id"]
                    cache_key   = layout_id if not locale_label else f"{locale}:{layout_id}"
                    plat        = layout["plataforma"]
                    out_dir = OUTPUT_DIR / marca_slug
                    for part in locale_path:
                        out_dir = out_dir / part
                    out_path = out_dir / plat / f"{layout_id}.png"

                    if solo_cambios and cache.get(cache_key) == h_new and out_path.exists():
                        print(f"  [skip] {marca_slug}/{locale_label}{layout_id} (sin cambios)")
                        skipped += 1
                        continue
                    ...
                    async def _render(
                        ...
                        hn=h_new,
                        ck=cache_key,
                        loc_label=locale_label,
                        marca_loc=marca,
                    ):
                        async with semaphore:
                            out_w = lo["ancho"] * max(1, int(render_scale))
                            out_h = lo["alto"] * max(1, int(render_scale))
                            print(f"  [render] {marca_slug}/{loc_label}{lid}  {out_w}×{out_h}")
                            await render_one(browser, marca_loc, lo, op, vd, render_scale)
                            cache[ck] = hn

                    tasks.append(_render())
                    task_labels.append(f"{locale_label}{layout_id}")
...
Ejemplos:
  ...
  python render.py --marca prizma --solo-cambios
    ...
    parser.add_argument(
        ...
        help="Idioma a renderizar: es, en o all (default: es)",
    )
    parser.add_argument(
    ...
    locale_arg = args.locale.lower()
    if locale_arg == "all":
        locales = list(SUPPORTED_LOCALES)
    elif locale_arg in SUPPORTED_LOCALES:
        locales = [locale_arg]
    else:
        parser.error(f"--locale debe ser uno de: {', '.join(SUPPORTED_LOCALES)}, all")
