...
PRODUCT_IMAGE_SET = [
    ("01-banner-web.png", "web/og_general.png"),
    ("02-hero-slider.png", "web/web_hero.png"),
    ("03-card-producto.png", "web/og_product.png"),
]

LEGACY_PRODUCT_IMAGE_NAMES = {
...
def localized_url(marca: dict, locale: str) -> str:
    if locale == "en":
        return (
            marca.get("url_en")
            or marca.get("locales", {}).get("en", {}).get("url")
            or marca.get("url_producto_en")
            or marca.get("url_producto")
            or marca.get("url")
            or marca.get("url_corporativa")
            or ""
        )
    return marca.get("url_producto") or marca.get("url") or marca.get("url_corporativa") or ""


def localized_tagline(marca: dict, locale: str) -> str:
    if locale == "en":
        return marca.get("locales", {}).get("en", {}).get("tagline") or marca.get("tagline") or ""
    return marca.get("tagline") or ""
...
def sync_product_catalog() -> list[dict]:
    ...
    for marca_file in sorted(MARCAS.glob("*.json")):
        ...
        for old_name in LEGACY_PRODUCT_IMAGE_NAMES:
        ...
        url_es = localized_url(marca, "es")
        url_en = localized_url(marca, "en")
        tagline_es = localized_tagline(marca, "es")
        tagline_en = localized_tagline(marca, "en")
        readme = [
            f"# {marca.get('nombre_producto', slug)}",
            "",
            f"- Marca Eikon: `{slug}`",
            f"- Ecosistema: {marca.get('nombre_corporativo') or group}",
            f"- URL ES: {url_es or 'pendiente'}",
            f"- URL EN: {url_en or 'pendiente'}",
            f"- Tagline ES: {tagline_es or 'pendiente'}",
            f"- Tagline EN: {tagline_en or 'pendiente'}",
            "",
            "## Imagenes base por uso UI",
            "",
            "| Uso | Archivo |",
            "|---|---|",
            "| Banner web / share | `imagenes/01-banner-web.png` |",
            "| Hero / slider | `imagenes/02-hero-slider.png` |",
            "| Card de producto | `imagenes/03-card-producto.png` |",
            "",
        ]
...
def write_product_index(rows: list[dict]) -> None:
    lines = [
        ...
        "",
        "Generado desde Eikon. Cada producto tiene minimo 3 imagenes base por uso UI:",
        "",
        "- `01-banner-web.png`",
        "- `02-hero-slider.png`",
        "- `03-card-producto.png`",
        "",
        f"Total productos: {len(rows)}",
        "",
    ]
    for group in sorted({row["group"] for row in rows}):
        lines += [f"## {group}", "", "| Producto | Marca | URL ES | URL EN | Imagenes |", "|---|---|---|---|---|"]
        for row in [r for r in rows if r["group"] == group]:
            rel = f"{row['group']}/{row['slug']}"
            url_es = row["url_es"] or "pendiente"
            url_en = row["url_en"] or "pendiente"
            lines.append(f"| {row['name']} | `{row['slug']}` | {url_es} | {url_en} | [{rel}]({rel}/) |")
        lines.append("")
