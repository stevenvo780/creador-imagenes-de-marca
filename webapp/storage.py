...
def create_tenant_user(db_path: Path, tenant_slug: str, tenant_name: str, email: str, password: str) -> dict[str, Any]:
    now = int(time.time())
    with connect(db_path) as con:
        con.execute("BEGIN")
        con.execute("INSERT INTO tenants(slug, name, created_at) VALUES (?, ?, ?)", (tenant_slug, tenant_name, now))
        ...
        con.commit()
    return {"tenant_id": tenant_id, "user_id": user_id, "tenant_slug": tenant_slug, "email": email.lower(), "role": "owner"}
...
def authenticate_user(db_path: Path, email: str, password: str) -> dict[str, Any] | None:
    ...
    if row is None or not verify_password(password, row["password_hash"]):
        return None
    return {"user_id": row["id"], "tenant_id": row["tenant_id"], "tenant_slug": row["tenant_slug"], "email": row["email"], "role": row["role"]}
...
def create_job(db_path: Path, tenant_id: int, user_id: int, marca_slug: str, category: str | None, dry_run: bool, params: dict[str, Any] | None = None) -> dict[str, Any]:
    now = int(time.time())
    with connect(db_path) as con:
        con.execute(
            "INSERT INTO jobs(tenant_id, user_id, marca_slug, category, dry_run, status, params_json, created_at) VALUES (?, ?, ?, ?, ?, 'queued', ?, ?)",
            (tenant_id, user_id, marca_slug, category, 1 if dry_run else 0, json.dumps(params or {}, sort_keys=True), now),
        )
...
def update_job_status(db_path: Path, job_id: int, status: str, result_summary: dict[str, Any] | None = None) -> None:
    with connect(db_path) as con:
...
def get_job(db_path: Path, tenant_id: int, job_id: int) -> dict[str, Any] | None:
    with connect(db_path) as con:
        row = con.execute("SELECT * FROM jobs WHERE tenant_id = ? AND id = ?", (tenant_id, job_id)).fetchone()
    return row_to_dict(row)


def list_jobs(db_path: Path, tenant_id: int, limit: int = 50) -> list[dict[str, Any]]:
    with connect(db_path) as con:
        rows = con.execute("SELECT * FROM jobs WHERE tenant_id = ? ORDER BY id DESC LIMIT ?", (tenant_id, limit)).fetchall()
    return [dict(r) for r in rows]


def add_asset(db_path: Path, tenant_id: int, job_id: int | None, marca_slug: str, category: str, type_name: str, variant: str, output_path: str, **extra: Any) -> dict[str, Any]:
    now = int(time.time())
    with connect(db_path) as con:
        con.execute(
            ...
            (tenant_id, job_id, marca_slug, category, type_name, variant, output_path, extra.get("size_bytes"), extra.get("hash"), extra.get("layout_status"), now),
        )
...
def list_assets(db_path: Path, tenant_id: int, marca_slug: str | None = None, limit: int = 100) -> list[dict[str, Any]]:
