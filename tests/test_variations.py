...
def test_deterministic_seed_stable() -> None:
    ...
    check("seed cabe en uint64 (>= 0 y < 2**64)", 0 <= s1 < 2**64, f"seed={s1}")
...
def test_deterministic_seed_distinct() -> None:
    ...
    for changed in ("marca", "category", "type", "variant", "idx", "salt"):
        ...
        check(f"cambiar {changed!r} → seed distinto", s_other != s_base, f"both={s_base}")
...
def test_variation_request_basic() -> None:
    ...
    d = req.as_dict()
    check(
        "as_dict() tiene claves esperadas",
        {"marca", "category", "type", "count", "base_variants", "strategy"}.issubset(d.keys()),
    )
...
def test_validate_request_errors() -> None:
    ...
    base_kw = dict(marca="prizma-iris", category="banners", type="ad_leaderboard", count=10)
    ...
    cases: list[tuple[str, dict, str]] = [
        ...
        ("axis con 0 opciones", {"base_variants": {"color": ()}}, "axis"),
        ("axis con opción no-str", {"base_variants": {"color": ("a", 1)}}, "axis"),
    ]
...
def test_expand_count_respected() -> None:
    ...
    for n in (1, 50, 200):
        ...
        check(f"count={n} → idx cubren 0..{n - 1}", [v.idx for v in plan] == list(range(n)))
...
def test_expand_deterministic() -> None:
    ...
    check(
        "mismos args → mismos variants", [v.variant_str for v in p1] == [v.variant_str for v in p2]
    )
...
def test_expand_seeds_unique_in_plan() -> None:
    ...
    check("50 seeds, todos distintos", len(set(seeds)) == 50, f"unique={len(set(seeds))}/50")
...
def test_expand_seeded_strategy_diversity() -> None:
    ...
    check(
        "5 variations todas distintas (cartesiano 3×2=6 >= 5)",
        len(set(variants)) == 5,
        f"got {variants}",
    )
...
def test_expand_round_robin_strategy() -> None:
    ...
    check(
        "round_robin cicla: [v1,v2,v3,v1,v2,v3]",
        colors == ["v1", "v2", "v3", "v1", "v2", "v3"],
        f"got {colors}",
    )
...
def test_expand_empty_axes() -> None:
    ...
    check("todas las variations tienen params={}", all(v.params_dict() == {} for v in plan))
    seeds = [v.seed for v in plan]
    check(
        "seeds siguen siendo distintos (por idx)",
        len(set(seeds)) == 5,
        f"unique={len(set(seeds))}/5",
    )
...
def test_expand_single_axis_single_option() -> None:
    ...
    check("todas params={color: only}", all(v.params_dict() == {"color": "only"} for v in plan))
    check("idx son 0..3", [v.idx for v in plan] == [0, 1, 2, 3])
...
def test_plan_protocol() -> None:
    ...
    check("plan[0] es Variation", isinstance(plan[0], variations.Variation))
    check("plan[-1] es Variation (índice negativo)", isinstance(plan[-1], variations.Variation))
    check("plan[0].idx == 0", plan[0].idx == 0)
    check("plan[-1].idx == 6", plan[-1].idx == 6)
    check("iter(plan) yields 7 items", len([v for v in plan]) == 7)
...
def test_plan_to_jsonl() -> None:
    ...
    check("cada línea parseable a dict", all(isinstance(p, dict) for p in parsed))
    check("cada dict tiene 'seed' y 'params'", all("seed" in p and "params" in p for p in parsed))
    check("orden de idx es 0,1,2", [p["idx"] for p in parsed] == [0, 1, 2])
...
def test_plan_to_dict_serializable() -> None:
    ...
    d = plan.to_dict()
    check("top-level tiene 'request' y 'variations'", "request" in d and "variations" in d)
    check("request.marca == prizma-iris", d["request"]["marca"] == "prizma-iris")
    check("request.strategy == 'seeded'", d["request"]["strategy"] == "seeded")
    check("2 variations en dict", len(d["variations"]) == 2)
    j = json.dumps(d, ensure_ascii=False, sort_keys=True)
    check("to_dict() round-trip JSON OK", isinstance(j, str) and len(j) > 0)
...
def test_separate_marcas_different_plans() -> None:
    ...
    kw = dict(
        count=10, category="banners", type="ad_leaderboard", base_variants={"color": ("a", "b")}
    )
    p1 = variations.expand_variations(marca="prizma-iris", **kw)
    p2 = variations.expand_variations(marca="pinakotheke-kosmos", **kw)
    check("seeds prizma-iris ≠ pinakotheke-kosmos", [v.seed for v in p1] != [v.seed for v in p2])
    check(
        "marca persistida en cada Variation",
...
def test_separate_salts_different_plans() -> None:
    ...
    check("seeds saltA ≠ saltB", [v.seed for v in pA] != [v.seed for v in pB])
...
def test_real_case_50_banners_prizma_iris() -> None:
    ...
    check("todas las variations son categoría=banners", all(v.category == "banners" for v in plan))
    check("todas son tipo=ad_leaderboard", all(v.type == "ad_leaderboard" for v in plan))
    check("todas son marca=prizma-iris", all(v.marca == "prizma-iris" for v in plan))
    check("idx cubren 0..49", [v.idx for v in plan] == list(range(50)))
    # Cartesiano = 3×3×2 = 18 → no podemos cubrir 18 únicos con 50 (ciclo).
    all_params = set(v.variant_str for v in plan)
    check(
        "variants observados ≤ 18 (cartesiano)",
        len(all_params) <= 18,
        f"observed {len(all_params)}",
    )
    check(
        "variants observados >= 2 (no degenerado)",
        len(all_params) >= 2,
        f"observed {len(all_params)}",
    )
    check("seeds son únicos en el plan", len({v.seed for v in plan}) == 50)
    check("re-correr produce seeds idénticos", [v.seed for v in plan] == [v.seed for v in plan2])
    check("re-correr produce plan dataclass-equal", plan == plan2)
...
def test_standard_categories_completeness() -> None:
    ...
    check(
        "categorías canónicas presentes",
...
def test_module_metadata() -> None:
    ...
    check(
        "__version__ es str no-vacío",
        isinstance(variations.__version__, str) and len(variations.__version__) > 0,
    )
    check(
        "__all__ contiene símbolos públicos clave",
        {
            "VariationRequest",
            "Variation",
            "VariationPlan",
            ...
            "validate_request",
            "STANDARD_CATEGORIES",
        }.issubset(set(variations.__all__)),
    )
